...
router.get("/update-voucher-bookings", function (req, res) {
  ...
  connectionMain.connect(function (err) {
    ...
    connectionMain.query(
      "SELECT * FROM voucher_list vl LEFT JOIN booking b ON vl.booking_id = b.id",
      function (err, vouList) {
        if (err) {
          connectionMain.end();
          return res.json({ success: false, error: err });
        }
        ...
        connectionMain.beginTransaction(function (err) {
          ...
          async.forEachSeries(
            vouList,
            function (voucherRecord, next) {
              const bookingId = voucherRecord.booking_id;

                // Update corporate booking
                connectionMain.query(
                  `UPDATE booking
                    SET is_corporate_issued_voucher = 1
                    WHERE id = ?`,
                  [bookingId],
                  function (err) {
                    if (err) {
                      errors.push({ bookingId: bookingId, error: err });
                      return next(err);
                    }
